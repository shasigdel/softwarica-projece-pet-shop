<!DOCTYPE html>
<html>
<head>
	<title>Lucy Pet Shop</title>
	<link rel="stylesheet" href="css/index.css" type = "text/css">
</head>
<script>
		function fun1(){
			alert ("Login First");
		}
	</script>
<body>

<!-- CONTAINER -->
	<div id = "container">
	<!-- HEADER -->
		<div id = "header">
			<div id ="logo">
			<img src="img/logo.png" href = "index.php">
			</div>
			
			<div id = "navbar">
				<ul>
					<li><a href="index.php">Home</a></li>
					<li><a href="member.php">Members</a></li>
					<li><a href="help.php">Help</a></li>
					<li><a href="about.php">About</a></li>
				</ul>
			</div>
			
			<div id = "top_info">Lucy's Pet Services</div>
			</div>
				<div id= "banner"><marquee scrollamount = "20"> <img src="img/run.gif" ></marquee>
			</div>
			<div id= "marquee">
			<marquee>Contact us at Lucypet@services.com or 9854879512</marquee></div>
						<div id = "ser"><div id = "services">Servces we provide
				<div id = "list">
				<ul>
					<li><a href="#" onclick="fun1()">Walking Pets</a></li>
					<li><a href="#" onclick="fun1()">Feeding Pets</a></li>
					<li><a href="#" onclick="fun1()">Puppy Bonding Session</a></li>
					<li><a href="#" onclick="fun1()">Play Sessions</a></li>
				</ul>
					
				</div>
			</div>
			</div>
			
			<div id = "footer">Shashank <br>
	Copyright @2016 <br>
	<strong>Contact E-mail:</strong> <br>
	lucypet@services.com
	</div>
	</div>